...
class OpenChequePaymentSuccessDialog extends StatelessWidget {
  final ScreenshotController screenshotControllerOpenCheque;

  const OpenChequePaymentSuccessDialog({super.key, required this.screenshotControllerOpenCheque});
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  ...
                  style: mediumText.copyWith(fontSize: 30.sp, color: goSmartBlue, fontWeight: FontWeight.bold),
                ),
                const Spacer(),
                TextButton.icon(
                  ...
                  // onLongPress: () {
                  //   context.read<OpenChequeProvider>().resetAfterFinishOrder(context);
                  //   //*
                  //   Navigation().closeDialog(context);
                  //   Navigation().closeDialog(context);
                  // },
                  onPressed: () async {
                    if (context.read<PosProvider>().usePrinter) {
                      ...
                      if (printName == '') {
                        ...
                        showDialog(
                            ...
                            builder: (context) => OkDialog(
                                  ...
                                  onPressed: () {
                                    context
                                        .read<OpenChequeProvider>()
                                        .resetAfterFinishOrder(openChequeProviderWatch.selectedTableId);
                                    //*
                                    Navigation().closeDialog(context);
                                    Navigation().closeDialog(context);
                                  },
                        ...
                        try {
                          await context
                              .read<PosProvider>()
                              .printReceipt(screenshotControllerOpenCheque, context.read<PosProvider>().isUSBPrinter);
                        } catch (e) {
                          print(e);
                        }

                        //     await Future.delayed(const Duration(seconds: 1), () {
                        context.read<OpenChequeProvider>().resetAfterFinishOrder(openChequeProviderWatch.selectedTableId);
                        //   });
                        //*
                        Navigation().closeDialog(context);
                        Navigation().closeDialog(context);
